...
{ 
    /* The char is stored at left and padd at buffer's right */
	int i = 0;
	char padd = ' ';
 ...
 * @is_negative: This is the list of arguments
 * @ind: The char types.
 * @buffer: The buffer array to handle print
...
/**
 * write_num - The writes a number using a bufffer
	...
	int length, char padd, char extra_c)
{
	...
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0' && width == 0)
		return (0); /* The printf(".0d", 0)  no char is printed */
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* The assign extra char to left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[ind], length) + write(1, &buffer[1], i - 1));
		}
		else if (!(flags & F_MINUS) && padd == ' ')/* The extra char to left of buff */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[1], i - 1) + write(1, &buffer[ind], length));
		}
		else if (!(flags & F_MINUS) && padd == '0')/* The extra char to left of padd */
		{
			if (extra_c)
...
/**
 * write_unsgnd - The writes an unsigned number
	...
	int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* The assign extra char to left of buffer [buffer>padd]*/
		{
			return (write(1, &buffer[ind], length) + write(1, &buffer[0], i));
		}
		else /* The assign extra char to left of padding [padd>buffer]*/
		{
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')/* The extra char to left of buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
